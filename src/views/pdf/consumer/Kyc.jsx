...
const Kyc = () => {
  ...
  const subjectlist = kyc.map(item => item.SubjectList).filter(Boolean);
  const personaldetailsssummary = kyc.map(item => item.PersonalDetailsSummary).filter(Boolean);
  const identificationHistory = kyc.map(item => item.IdentificationHistory).filter(Boolean);
  const addressHistory = kyc.map(item => item.AddressHistory).filter(Boolean);
  const employmentHistory = kyc.map(item => item.EmploymentHistory).filter(Boolean);
  const telephoneHistory = kyc.map(item => item.TelephoneHistory).filter(Boolean);
  const enquiryDetails = kyc.map(item => item.EnquiryDetails).filter(Boolean);
  ...
  console.log("empl", EmploymentHistory)
  console.log("tell,", TelephoneHistory)
  console.log("addres,", AddressHistory)
  return (
